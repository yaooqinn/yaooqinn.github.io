...
在 TPC-H 和 TPC-DS(SF=3/10)上,论文报告**单 query 最高 4.78× speedup**[^2]——这个数字本身不是这篇博客的重点,我反而想说它的 **caveat**:
...
所以 4.78× 这个数字读法应该是:**"DataFusion 现有 physical optimizer 在这批 query 上的可改进空间"**,不是"LLM 比 cost-based optimizer 强"。这一点要先讲清楚,后面才好谈架构选择。
...
每一条 patch 是原子的、可以单独 revert、可以单独写测试用例。从工程治理角度,**这比"我们写了一条新 rule"或"agent 直接选了这个 join 顺序"更可控**——尤其是在 B 路线里,LLM agent 直接吐出"用这个 join 顺序",输出本身没有 diff,你只能信或不信。
...
要诚实地说一句:论文也没给"几次复用之后才回本"的实测数据,这是这条路线后续工作要补的。
...
这是一套**符合现有 Spark/Kyuubi 这类项目里 SQL gateway 工程实践的**机制。LLM 在这里的角色更像"补丁生成器",不是 runtime 上的在线决策者。

## 落到 Spark 上,挂点其实已经存在

如果想在 Spark 上把这条路线走通,API 层是现成的。`SparkSessionExtensions` 提供了 `injectPlannerStrategy`、`injectOptimizerRule`、`injectPostHocResolutionRule` 等扩展点[^4],其中 `injectPlannerStrategy` 注入的 `SparkStrategy` 会在 logical → physical 转换时介入——**正好是"optimizer 已经做完它的工作"之后、"执行真正开始"之前**的位置。一个 patch applier 自然该挂在这里。

真正的工程瓶颈不在 API,在**`SparkPlan` 和 JSON 之间的序列化/反序列化**。Catalyst 的 plan tree 节点没有官方的 JSON codec,patch 路线要落地就得先把这块基础设施做出来。这是任何"LLM 调 Spark plan"的工作绕不开的前置依赖,DBPlanBench 在 DataFusion 上写了一份扁平 node-id schema(节点 id + input/left/right 引用)可以作为参考。

## 顺便回答一个常见质疑

"那 LLM 选 plan 这件事,training-free 的版本是不是更便宜?"

有这么一篇工作:**LLM-PM** (arXiv 2506.05853)[^5] 用 `text-embedding-3-large` 把 EXPLAIN plan 文本编码成向量,query 来时去历史 plan 库里做 KNN 检索,选最相似的那条历史 plan 套用。完全 training-free,论文报告在 OpenGauss + JOB-CEB 上**均值 -21% latency**。

这看起来很有吸引力,但仔细看分布:**约 20% 的 query 反而被减速,60% 不变**[^5]。均值 -21% 是少数大幅加速 query 把均值拉低的结果。这是另一个独立的话题(下次单独写),这里只想点一句:**LLM-PM 是"在历史 plan 库里选已有",DBPlanBench 是"生成新 plan 结构"。前者无法创造新结构,后者可以——它们其实不是一类工作**。

均值掩盖分布,是 LLM × QO 这个领域目前公认的报告纪律问题。任何"我们均值 +N%"的论文都该被追问"加速 / 减速 / 不变三分布"。

## 收束

LLM 进查询优化器的讨论,过去常常停留在"它能不能替代 cost-based optimizer"。这个问题本身可能就问错了。

**把它放在 optimizer 之后,以 patch 形式做最后一公里微调,这条路线相对其它两条至少在三件事上更好谈**:

- 输出粒度小,review 友好
- 摊销模型清晰,OLAP 反复运行是天然契合
- 落地路径短,Spark/Kyuubi 这类项目里挂点已经存在

这不是说路线 C 一定赢,而是说**讨论应该在 C 路线的疑点上展开**(规模、跨引擎可移植性、patch 库的工程化代价),而不是停留在替代 vs 共存这种范式之争。

Apache Spark / Gluten / Kyuubi 这类项目里,如果有人想认真做一次实验,**优先级应该是先把 `SparkPlan ↔ JSON` 这层做出来**,后面 LLM、规则化、缓存,都是接得上去的。
