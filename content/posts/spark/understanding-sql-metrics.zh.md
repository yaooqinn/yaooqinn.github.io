...
REST API 和 Web UI 返回格式化字符串，不是原始数值：

**大小指标** — `%.1f` 四舍五入，GiB 级别可丢失 ~50 MiB 精度

**计时指标** — 递进式四舍五入：
```
1,234 ms → "1.2 s"      (丢失 34ms)
65,432 ms → "1.1 m"     (丢失 5.4 秒)
7,384,000 ms → "2.05 h" (丢失约 12 秒)
```

**nsTiming** — 纳秒先截断为毫秒，再四舍五入，双重精度损失
...
缺失：原始数值和指标类型。[spark-history-cli](https://github.com/yaooqinn/spark-history-cli) 等工具必须正则解析显示字符串。
...
两个自由字符串，无验证。存在不一致：`"data size (bytes)"` vs `"data size"`。

## 实用建议

1. **看 min/med/max，不只是 total。** 分布才能揭示倾斜。
2. **没有计时 ≠ 很快。** Python UDF 和 Window 通常最慢但无默认计时。
3. **不要过度信任微小差异。** 四舍五入可导致 "1.5 GiB" 之间相差 150 MiB。
4. **Web UI 和 REST API 数据完全一致。** 同一数据源，无隐藏详情。
5. **WholeStageCodegen 改变可测量范围。** 代码生成管道内无法获得单算子执行时间。
