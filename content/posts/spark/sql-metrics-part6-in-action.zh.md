...
我们在一个集群上以 **SF10000**（10 TB 原始数据量）运行了这个查询，使用 **Gluten/Velox** 作为原生执行后端，底层存储为 **Microsoft Fabric OneLake 上的 Delta Lake 表**。
...
而且这 2.1 GiB 全部来自本地 SSD 缓存 — 没有一个字节需要通过网络访问 OneLake。
